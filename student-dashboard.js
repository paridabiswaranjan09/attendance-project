...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const selectedYear = localStorage.getItem("studentSelectedYear") || ""; // NEW: Year added
    let courseBranchYearKey = localStorage.getItem("studentCourseBranchYearKey") || ""; // UPDATED: Now includes year

    // FIXED: If courseBranchYearKey is empty but course/branch/year exist, create it automatically
    if (!courseBranchYearKey && selectedCourse && selectedBranch && selectedYear) {
        courseBranchYearKey = `${selectedCourse}_${selectedBranch}_${selectedYear}`.toUpperCase(); // e.g., "BTECH_CSE_2024"
        localStorage.setItem("studentCourseBranchYearKey", courseBranchYearKey); // Save it for future
        console.log("Auto-created courseBranchYearKey:", courseBranchYearKey);
    }

    // Display in profile - Enhanced with Year
    let displayText = "No course/branch/year selected";
    if (courseBranchYearKey) {
        if (selectedCourse && selectedBranch && selectedYear) {
            displayText = `${selectedCourse} - ${selectedBranch} - ${selectedYear}`;
        } else {
            displayText = courseBranchYearKey; // Fallback to key (e.g., "BTECH_CSE_2024")
        }
    }
    document.getElementById("studentCourseBranch").textContent = displayText; // UPDATED: Now shows year too

    // Display student name
    document.getElementById("studentNameDisplay").textContent = studentName;

    // Pre-fill complaint form
    const complaintStudentNameInput = document.getElementById("complaintStudentName");
    const complaintStudentRegdNoInput = document.getElementById("complaintStudentRegdNo");
    if (complaintStudentNameInput) complaintStudentNameInput.value = studentName;
    if (complaintStudentRegdNoInput) complaintStudentRegdNoInput.value = studentRegdNo;
    ...
    // If no course-branch-year after fix, redirect to login
    if (!courseBranchYearKey) {
        alert("No course/branch/year selected. Please login again and select properly.");
        // Clear invalid data
        localStorage.removeItem("studentSelectedCourse");
        localStorage.removeItem("studentSelectedBranch");
        localStorage.removeItem("studentSelectedYear"); // NEW: Clear year too
        localStorage.removeItem("studentCourseBranchYearKey");
        window.location.href = "Login-Table.html";
        return;
    }

    // Global variables for this dashboard - UPDATED with year
    window.studentCourseBranchYearKey = courseBranchYearKey; // Renamed for clarity
    window.studentRegdNo = studentRegdNo;

    // Enhanced Debug: Log to console (remove in production)
    console.log("Student Dashboard Loaded (With Year):", {
        studentName,
        studentRegdNo,
        selectedCourse,
        selectedBranch,
        selectedYear,  // NEW
        courseBranchYearKey,  // Now includes year
        displayText
    });
    console.log("Full LocalStorage Keys:", Object.keys(localStorage).filter(key => key.startsWith("student")));
});
...
document.getElementById("logoutBtn").addEventListener("click", () => {
    // Clear student data - UPDATED with year
    localStorage.removeItem("studentName");
    localStorage.removeItem("studentRegdNo");
    localStorage.removeItem("studentSelectedCourse");
    localStorage.removeItem("studentSelectedBranch");
    localStorage.removeItem("studentSelectedYear"); // NEW
    localStorage.removeItem("studentCourseBranchYearKey"); // UPDATED key name
    window.location.href = "Login-Table.html";
});

function hideAllSections() {
    if (viewAttendanceSection) viewAttendanceSection.style.display = "none";
    if (applyComplaintSection) applyComplaintSection.style.display = "none";
}
...
function loadStudentAttendanceTable() {
    const courseBranchYearKey = window.studentCourseBranchYearKey; // UPDATED: Use year key
    const regdNo = window.studentRegdNo;
    if (!courseBranchYearKey || !regdNo) {
        studentAttendanceTableBody.innerHTML = '<tr><td colspan="4" style="text-align:center;">No data available. Please login again.</td></tr>';
        if (downloadStudentPdfBtn) downloadStudentPdfBtn.disabled = true;
        return;
    }
    ...
    // Load attendance records for this date - UPDATED key with year
    const records = JSON.parse(localStorage.getItem(`attendance_${courseBranchYearKey}_${date}`) || "[]");
    ...
    // Calculate monthly percentage for this student - UPDATED with year
    ...
    studentAttendanceTableBody.innerHTML = `
        <tr>
            <td>${localStorage.getItem("studentName") || 'Student'}</td>
            <td>${regdNo}</td>
...
function calculateStudentMonthlyPercentage() {
    const courseBranchYearKey = window.studentCourseBranchYearKey; // UPDATED
    const regdNo = window.studentRegdNo;
    if (!courseBranchYearKey || !regdNo) return 0;
    ...
    // Scan all dates in current month - UPDATED key with year
    for (let i = 1; i <= 31; i++) {
        const dateStr = `${currentYear}-${String(currentMonth).padStart(2, '0')}-${String(i).padStart(2, '0')}`;
        const records = JSON.parse(localStorage.getItem(`attendance_${courseBranchYearKey}_${dateStr}`) || "[]");
        const record = records.find(r => r.regdNo === regdNo);
        if (record) {
            totalCount++;
...
// PDF Download for Student - UPDATED with Year in Header & Filename
function downloadStudentAttendancePDF() {
    ...
    const year = localStorage.getItem("studentSelectedYear") || ""; // NEW: Year in PDF
    const studentName = localStorage.getItem("studentName") || "";
    const regdNo = window.studentRegdNo;
    const courseBranchYearKey = window.studentCourseBranchYearKey;
    ...
    const filename = `student_attendance_${course.replace(/ /g, '_')}_${branch.replace(/ /g, '_')}_${year}_${date}.pdf`;
    doc.save(filename);
}
...
// (Same as before, but UPDATED key with year)
...
function saveComplaint(text, photoBase64) {
    const courseBranchYearKey = window.studentCourseBranchYearKey; // UPDATED: With year
    const studentName = localStorage.getItem("studentName");
    const regdNo = window.studentRegdNo;
    const course = localStorage.getItem("studentSelectedCourse");
    const branch = localStorage.getItem("studentSelectedBranch");
    const year = localStorage.getItem("studentSelectedYear"); // NEW: Year in complaint
    const timestamp = new Date().toISOString();

    // Load existing complaints - UPDATED key with year
    let complaints = JSON.parse(localStorage.getItem(`complaints_${courseBranchYearKey}`) || "[]");

    // Add new complaint - UPDATED with year
    complaints.push({
        name: studentName,
        regdNo: regdNo,
        course: course,
    ...
    // Save back
    localStorage.setItem(`complaints_${courseBranchYearKey}`, JSON.stringify(complaints));

    // Clear form
    if (complaintText) complaintText.value = "";
    if (complaintPhoto) complaintPhoto.value = "";
