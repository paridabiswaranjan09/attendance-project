const API_URL = "http://localhost:5000/api/admin"; // ✅ Backend ka URL
...
document.getElementById('adminLoginBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            showFloatingMessage(data.message || "Invalid username or password");
        }
    } catch (err) {
        console.error(err);
        showFloatingMessage("Server error during login!");
    }
});
...
document.getElementById('createAccountBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            showFloatingMessage(data.message || "Signup failed!");
        }
    } catch (err) {
        console.error(err);
        showFloatingMessage("Server error during signup!");
    }
});
